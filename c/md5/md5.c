 ...
 * *********************************************************/
void EncodeBit(unsigned char *output,unsigned int *input,unsigned int len)  
{  
...
void DecodeBit(unsigned int *output,unsigned char *input,unsigned int len)  
{  
    unsigned int i = 0,j = 0;  
    while(j < len)  
    {  
...
{  
    ...
    EncodeBit(bits,context->count,8);
    ...
    EncodeBit(digest,context->state,16);  
}    
...
/* function: MD5Encode
 * parameter: encrypt（需要加密的字符串），encodeBit（加密后的位数，16和32位），result（加密后的字符串）
*/ result 数值长度，16为一定要大于16,32为一定要大于32；result[64]
void MD5Encode(unsigned char *encrypt, unsigned int encodeBit, unsigned char *result)
{
	int i;
	MD5_CTX md5; 
    MD5Init(&md5); 
    ...
    if(16 == encodeBit)
    {
	    ...
	    {  
	    	if(i == 4)
	    	{
	    		sprintf(result, "%02x", (unsigned char)decrypt[i]);
	    	}
	    	else
	    	{
	    		sprintf(result, "%s%02x", result, (unsigned char)decrypt[i]);
	    	}
	    }
    }
    else if(32 == encodeBit)
    {
    	for(i = 0; i < 16; i++)  
    	{
    		if(i == 0)
